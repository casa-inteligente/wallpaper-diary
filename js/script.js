...
const dayArr=["Domingo","Segunda - Feira", "Terça - Feira", "Quarta - Feira", "Quinta - Feira", "Sexta - Feira", "Sabado"];
...
function UpdateClock() {
  ...
  */
  ...
  if(_12hour)
    timeEl.innerHTML=`- ${new Intl.DateTimeFormat('pt-BR',{'hour':'2-digit','minute':'2-digit','hour12':true}).format(d)} -`.replace("AM",'').replace("PM",'');
  else
    timeEl.innerHTML=`- ${new Intl.DateTimeFormat('pt-BR',{'hour':'2-digit','minute':'2-digit','hour12':false}).format(d)} -`;

  if(mmddyy)
    dateEl.innerText=new Intl.DateTimeFormat('pt-BR',{'day':'2-digit','month':'short','year':'2-digit'}).format(d).replace(',','');
  else
    dateEl.innerText=new Intl.DateTimeFormat('pt-BR',{'day':'2-digit','month':'2-digit','year':'2-digit'}).format(d).replace(',','');
